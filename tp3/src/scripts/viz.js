...
export function setColorScaleDomain (colorScale, data) {
  // Getting the max value possible in our domain.
  let maxValueCount = d3.max(
    data.map(
      (element) => {
  ...
  // Update our colorScale accordingly.
  colorScale
    .domain([0,maxValueCount])
}
...
export function updateXScale (xScale, data, width, range) {
  ...
  xScale
    .domain(range(minValueYear,maxValueYear))
    .range([0,width])
}
